...
def resolve(bet: dict, game: dict) -> str:
    ...
    Args:
        ...
        game: A final game dict from one of the result sources —
              odds_api.get_game_result() (team sports), espn_tennis.
              get_match_result() (tennis), or espn_fights.get_fight_result()
              (MMA/boxing) — all returning the same shape
    ...
    if bet_type == BET_TYPE_MONEYLINE:
        ...
        # Team totals ("Braves Team Total Over 4.5") settle against ONE team's
        # score, so they're tried FIRST; a plain game total ("Over 8.5") falls
        # through to the two-team sum below.
        team_total = _parse_team_total_selection(selection)
        if team_total is not None:
            team_part, direction, line = team_total
            return _resolve_team_total(team_part, direction, line, team1, team2,
...
# ── Team total ────────────────────────────────────────────────────────────────
# Machine-generated by odds-tool (positionKelly.mjs) as
# "{TeamShort} Team Total {Over|Under} {line}", logged as Bet Type Total. The
# same pattern market_key_infer.py keys on — one regex sensibility, not two.
_TEAM_TOTAL_RE = re.compile(
    r"^(.+?)\s+Team Total\s+(Over|Under)\s+([\d.]+)$", re.IGNORECASE
)


def _parse_team_total_selection(selection: str):
    """Return (team_part, direction_lower, line) for a team-total selection, or
    None when it isn't one (so the caller falls through to plain game-total
    parsing). A malformed team-total never matches → None → game-total path,
    which itself fails toward manual if that doesn't parse either."""
    m = _TEAM_TOTAL_RE.match(selection.strip())
    if not m:
        return None
    return m.group(1).strip(), m.group(2).strip().lower(), float(m.group(3))


def _resolve_team_total(team_part, direction, line, team1, team2,
                        home_team, away_team, home_score, away_score) -> str:
    """
    Settle a single team's total against THAT team's own score (not the game
    sum). Both teams are frequently quoted at the same line, so the side is
