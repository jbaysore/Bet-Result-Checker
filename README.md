...
the workflow. This mirrors the same setup used by the separate Closing Odds
Importer project.
...
| File | Purpose |
|---|---|
| `trigger.py` | Entry point. Loads pending bets, calls `poller.py` for each, then runs the P/L completion pass. |
...
| `sheets_reader.py` | All reads from Google Sheets (pending bets, unresolved P/L rows, Promotions' Boost %, Book Settings' fee policies). |
| `sheets_writer.py` | All writes to Google Sheets (`write_result`, `write_pl_payout`), each with a BetID-match safety check before writing. |
